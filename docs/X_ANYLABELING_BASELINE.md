...
- LabelMe JSON 与 X-AnyLabeling 目录的健壮双向互操作：可导入图片与同名 JSON；传统两点和可证明轴对齐的四角 rectangle 可编辑，旋转/退化四点及其他不支持 shape 连同扩展字段不静默丢失；损坏文件不崩溃，重开和从 DatumDock 导出后可由 X-AnyLabeling 再次打开。
