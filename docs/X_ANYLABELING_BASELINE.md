...
- 多工作区/多项目/多数据集快速切换，数据池、标签集、模型库严格隔离；新建数据集可复用兼容配置，标签集/数据集合并先经训练映射比较与差异确认。
- 受管数据集池导入、搜索、筛选、图片级复核状态和稳定样本 ID；至少一万张常见图片通过索引、虚拟列表、延迟缩略图和后台任务保持可用。
- 常见静态图片导入统一转换为 PNG、完全重复图片对比确认、近似图片相似组与不可拆分导出、少量样本回收站/大批量永久删除、项目备份校验和标签集一致的数据集转移。
- 命名规则预览与安全批量重命名；删除样本时清理受管关联信息且不删除外部源文件。
- 项目级标签管理、中文别名/描述/同义词、跨数据集标签检查集合和历史标签迁移。
...
- ONNX 模型的可靠本地运行与检测矩形框预标注；优先使用可用 GPU、首次 CPU 回退有可操作指引。结果直接进入项目并带模型来源、置信度和图片级待人工复核状态，可逐图审查、修改或删除。
...
- 每次发布前进行手工冒烟测试：创建/打开项目、导入、标注、标签检查、重命名、删除、模型预标注、导出和语言切换。
- 发现数据损坏、导出错误、静默丢失标注或 AI 覆盖人工标注，视为阻断发布的 P0 缺陷。
- 发布说明必须注明当前完成等级、已支持的任务/格式/模型以及已知限制。

## English Summary

DatumDock uses X-AnyLabeling as a workflow and quality benchmark, not as an undefined promise to clone every upstream feature. Completion levels are: L1 managed datasets and YOLO Detection; L2 reliable core image annotation workflows, including reopenable X-AnyLabeling/LabelMe exchange directories and preservation of unsupported shapes; L3 reviewed AI assistance and common interchange formats; and L4 separately accepted advanced tasks such as segmentation, pose, OBB, video, tracking, and OCR. A release may claim core workflow replacement only after every required item for the relevant level is verified.
