...
- [x] X-AnyLabeling 导入标签映射已修复：读取根目录 `labels.txt`，未知标签默认新建，非训练名安全转换并保留原别名，映射表不再裁切；导入矩形可编辑、改派和重启恢复。
- [x] 修复外部矩形双击标签改派保存检查点与错误分类；最近成功创建或改派的标签按数据集会话隔离，并成为下一框默认标签。
...
- [-] 添加应用图标、版本信息和 Windows 资源文件：双色 `DD` 多尺寸 PNG/ICO、源码窗口和任务栏绑定已完成；EXE、开始菜单、桌面快捷方式、卸载程序和版本资源仍待安装包阶段验证。
...
- [x] 步骤五 Python 3.11 与资料库隔离复验完成。
  - 验证结果：Ruff、格式、`compileall` 和完整 pytest 已有通过基线；2026-07-21 稳定化测试清单扩充为 248 项，最终连续两轮结果记录在 `docs/STABILIZATION_AUDIT.md`。Pillow 弃用警告已经清零；普通模式与预览模式继续使用隔离入口。
...
- [-] 本机分支覆盖率报告受可信依赖 TLS 阻塞。
  - 原因：`pytest-cov` 已加入 `.[dev]` 和 Windows CI，但 2026-07-21 从官方 PyPI 安装时持续发生 TLS EOF；未关闭 TLS 校验，当前环境也没有可信离线 wheel。
