...
- [-] 实现穷举式 `ActionRegistry`、快捷键设置页面、冲突检测、显式未绑定、即时应用和原子恢复全部默认按键：预览会话交互完成，真实全量操作注册、偏好保存与故障回滚待接入。
...
## 阶段 3.1：标注画布交互统一整改（需求整理中）

> 2026-07-19：当前仅整理需求文档，代码保持不变。等待用户继续汇总整改项并统一确认后，再集中实施和回归，避免在交互规则仍变化时反复修改。
...
- [ ] 汇总并锁定剩余整改需求，决定 `Ctrl + 滚轮` 是否保留缩放。
- [ ] 确认标签小窗是否记忆上次尺寸，以及新建标签后取消外层小窗时是否保留该标签。
- [ ] 实现一次性矩形状态机、两种创建手势与 `Esc` 无副作用取消。
- [ ] 为两次单击模式实现首点锚定、动态轮廓/宽高、任意方向坐标规范化和零面积重试。
- [ ] 实现图片边界内十字辅助线和集中式框线/填充透明度令牌。
- [ ] 实现中键平移及竖直/水平滚动映射，并处理边界、空图片和工具状态隔离。
- [ ] 实现双击标签小窗、响应式流式布局、搜索/键盘导航、快速创建及确认后单次自动保存。
- [ ] 统一画布与右侧列表的标注删除命令，补充文本焦点防穿透、撤销和最后一框状态转换。
- [ ] 扩展画布视图缩放范围至至少 `6400%`，增加比例输入、`100%`/适配恢复及高倍率渲染策略。
- [ ] 验证高倍率不创建巨型图片副本，且坐标命中、框编辑、辅助线、平移滚动和 LabelMe 保存无漂移。
- [ ] 将本轮新增的 `R`、删除、搜索、缩放和导航操作全部纳入 `ActionRegistry`，接入设置页、冲突检测和即时刷新。
- [ ] 实现“恢复全部默认按键”的影响预览、原子清除覆盖、失败回滚、重启持久化及与其他设置/数据集隔离测试。
- [ ] 将 `A / D` 图片导航接入浏览作用域、文本焦点防穿透、自动保存离开保护和首尾边界反馈测试。
- [ ] 将 SQLite v2 五状态事务迁移到 v3 双状态，保留无标注空值和独立健康诊断，并验证旧数据映射与回滚。
- [ ] 实现待复核图片“第一次有效人工编辑保存成功即自动完成”，并让创建/移动/缩放/删除/换标签/有效撤销重做与状态在同一事务提交。
- [ ] 实现 `review.mark_completed` 按钮与可配置快捷键，供人工检查后无需修改的图片明确完成。
- [ ] 更新状态筛选、图片徽标、主页进度、标签检查和模型自动标注，只显示待复核/已完成并移除旧状态入口。
- [ ] 补充单元测试、pytest-qt 交互回归、双语截图和三种目标分辨率视觉复验。
- [ ] 全部整改通过验收后，再更新 A0.4 与步骤四交互说明为已完成。
...
Step four's five-state review implementation remains historical evidence. In the pending two-state revision, the first effective manual annotation edit automatically completes a model-pending image after atomic save; explicit confirmation covers no-edit review. Unannotated and unhealthy samples remain separate conditions; implementation is pending.
