...
### A0.4 标注交互整改待验收（需求整理中）

> 本节记录用户在步骤四完成后提出的新交互要求。A0.3 的已勾选项目是旧行为的历史验证证据，不代表本节已经实现；本节完成后，新输入映射优先于旧的“滚轮缩放/连续画框”行为。

- [ ] 按 `R` 进入一次性矩形创建模式；成功创建一个框后自动回到选择/编辑模式。
- [ ] 同一个一次性矩形状态机同时支持左键拖拽释放，以及“第一次单击角点、第二次单击对角点”；两种手势生成完全一致的规范化像素坐标、历史节点和自动保存请求。
- [ ] 第一次单击后显示锚点、动态轮廓、实时宽高和辅助线；第二点位于任意方向时都能正确规范化矩形。
- [ ] 两次单击是两个独立点击事件；不会误触发已有矩形的双击改标签，也不会因系统双击间隔而提前完成。
- [ ] 第二点形成零面积时不生成 shape、历史节点、保存任务或复核变化，并允许重新指定第二点；`Esc` 可无副作用取消锚点。
- [ ] `Esc` 取消未完成的矩形并回到选择/编辑模式，不产生标注、历史节点、保存任务或复核状态变化。
- [ ] 指针在实际图片区域内时显示水平和竖直辅助线，离开图片、没有图片或中键平移时隐藏，且辅助线不越过图片边界。
- [ ] 矩形外框可见且不透明度高于内部填充；填充不遮挡关键图片细节，各标签继续使用自己的颜色。
- [ ] 鼠标中键按下拖拽可平移图片，松开后停止，且不改变当前选择或矩形工具状态。
- [ ] 普通滚轮只竖直滚动图片，`Alt + 滚轮` 只水平滚动图片，边界处正确停止。
- [ ] 工具栏按钮、缩放比例和适配窗口仍可用；`Ctrl + 滚轮` 的缩放映射在需求锁定前不纳入验收。
- [ ] 新交互不会破坏框选择、移动、八点缩放、自动保存、撤销/重做、图片切换和双数据集隔离。
- [ ] pytest-qt 覆盖一次性状态转换、两种创建手势、辅助线显隐、中键平移及两种滚轮方向；双语和目标分辨率截图通过。
- [ ] 双击画布矩形打开“更改标签”小窗并预选当前标签；打开、搜索和浏览候选不会提前修改矩形或触发保存。
- [ ] 小窗可从边缘和四角调整尺寸；标签卡片按可用宽度自动增减列数并换行，内容过多时只有中部标签区纵向滚动，顶部搜索和底部操作保持可见。
- [ ] 标签卡片显示颜色、中文别名、英文训练名及可到达的描述；当前项和候选项不只依赖颜色区分。
- [ ] 搜索可按别名、训练名、同义词和描述即时过滤；无结果状态、`Ctrl+F`、方向键、`Enter` 和 `Esc` 均可用。
- [ ] 小窗可快速新建真实数据集标签，复用完整标签管理的字段与唯一性规则；新标签创建后立即出现在候选区。
- [ ] 只有点击确认才产生一次标签改派、一个撤销历史节点和一次自动保存；取消或关闭时矩形标签和复核状态保持不变。
- [ ] 弹窗尺寸跨会话策略与“新建标签后取消外层小窗”的数据语义经用户确认，并有对应事务和回归测试。
- [ ] 单击右侧当前标注列表项后，列表项与画布框同步选中；列表或画布具有操作焦点时按 `Delete` 均删除同一条矩形标注。
- [ ] 右侧列表触发的删除只移除当前图片的标注实例，不删除数据集标签定义，也不影响其他图片中的同类标注。
- [ ] 列表删除只产生一个历史节点和一次自动保存；画布、列表、框数、反向索引和复核状态同步刷新，`Ctrl+Z` 可完整恢复。
- [ ] 搜索框、文本框、快速标签小窗或标签编辑表单具有文本焦点时，`Delete` 不会穿透并误删标注。
- [ ] 待复核图片删除最后一个矩形属于有效人工编辑；保存成功后自动变为已完成并以零框表达无目标，保存失败时仍为待复核。
- [ ] 图片可通过按钮和比例输入至少放大到 `6400%`，并可一键恢复 `100%` 或适配窗口；非法、非有限和越界比例被明确拒绝或钳制。
- [ ] 高倍率缩放只改变视图变换，不创建与放大尺寸等大的图片副本；压力测试证明内存不会按缩放倍率平方增长。
- [ ] 在 `100%`、`800%`、`3200%` 和 `6400%` 下，矩形坐标、鼠标命中、八点控制柄、十字辅助线和保存后的 LabelMe 像素坐标无可观察漂移。
- [ ] `6400%` 下仍能通过中键、滚轮和 `Alt + 滚轮` 到达图片四边与四角，框线、控制柄和标签不会大到遮挡主要目标。
- [ ] 高倍率插值策略和是否允许超过 `6400%` 经视觉/性能复验后锁定，并有对应截图或像素级测试。
...
The acceptance gate now requires one effective manual annotation edit to atomically complete a pending model result. View-only actions do not count, and a button or configurable shortcut completes no-edit review. The two-state schema and migration remain unchecked; no implementation is claimed.
