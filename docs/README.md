...
2. [产品需求文档](PRD.md)：用户需求、MVP 范围、数据格式、非功能性要求与待确认决策。
3. [架构说明](ARCHITECTURE.md)：领域对象、服务边界、受管存储、并发任务和数据安全规则。
4. [交互与界面规范](UX.md)：页面结构、标注操作、莫兰迪视觉系统、图标和交互反馈。
5. [代码规范](CODE_STYLE.md)：Ruff、测试、中文注释和 Python 编码约定。
6. [路线图](ROADMAP.md)：当前应执行的最小可验证任务。
7. [验收标准](ACCEPTANCE.md)：功能完成前必须满足的可操作或可自动验证条件。
8. [X-AnyLabeling 对标基线](X_ANYLABELING_BASELINE.md) 与 [互操作规范](X_ANYLABELING_INTEROP.md)：对外宣称兼容/对标前必须满足的质量与格式边界。
