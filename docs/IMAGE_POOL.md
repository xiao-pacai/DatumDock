...
> 状态：2026-07-19 步骤三已实现图片池，步骤四已验证其与真实标注协同。本文聚焦图片导入、索引、浏览、重命名和删除边界；标签、LabelMe 与复核详见 [标注工作流](ANNOTATION_WORKFLOW.md)。
...
- 图片池不自动删除、合并或拆分近似图。步骤六 `DeterministicSplitPlanner` 已使用稳定查询接口，把已确认相似组及其重叠关系合并为不可拆分连通分量，并保证组内样本进入同一个 train/val/test 子集。
...
图片导入本身不创建空 LabelMe JSON；步骤四在首次画框或确认零框完成时按需处理标注。步骤五的 X-AnyLabeling 目录导入复用同一 PNG 规范化、哈希、缩略图和完全重复判断，但同时发布配对 JSON 与反向标签索引；图片无 JSON 时仍保持无标注且复核状态为空。批量重命名、回收站恢复和永久删除继续同步维护标注文件名、`imagePath`、摘要与标签反向索引。

步骤五提交前记录外部源文件指纹，提交后复验来源树哈希不变。`xany_import` 操作日志纳入启动对账：已登记样本完成收尾，已发布但未登记的完整样本可恢复登记，无法证明完整的文件回到暂存并保留诊断。步骤六 YOLO Detection 导出只读使用图片池及其相似关系，输出到用户当次选择的新目录，不修改受管图片、标注、索引或相似状态。模型推理和备份仍在后续步骤。

## English Summary

Step 5 reuses the managed image pool for source-safe X-AnyLabeling directory ingestion, including PNG normalization, hashes, thumbnails, duplicates, paired annotations, and recoverable operation logs. External source files are never modified. Step 6 reads the image pool and similarity relationships to produce deterministic YOLO Detection splits without modifying managed data. Actual third-party X-AnyLabeling GUI verification remains open; model inference and backups remain later work.
