...
图片导入本身不创建空 LabelMe JSON；步骤四在首次画框或确认无目标负样本时按需创建。批量重命名、回收站恢复和永久删除已同步维护标注文件名、`imagePath`、摘要与标签反向索引。X-AnyLabeling 完整目录互操作、模型、YOLO 导出和备份仍在后续步骤。

## English Summary

DatumDock step three implements the managed image pool, and step four verifies its integration with persistent annotations. Rename, trash restore, and permanent deletion now keep LabelMe paths, imagePath, digests, and reverse label indexes consistent. External source files are never modified. Complete X-AnyLabeling directory interchange, models, exports, and backups remain later-stage work.
