...
步骤五提交前记录外部源文件指纹，提交后复验来源树哈希不变。`xany_import` 操作日志纳入启动对账：已登记样本完成收尾，已发布但未登记的完整样本可恢复登记，无法证明完整的文件回到暂存并保留诊断。模型、YOLO 导出和备份仍在后续步骤。

## English Summary

Step 5 reuses the managed image pool for source-safe X-AnyLabeling directory ingestion, including PNG normalization, hashes, thumbnails, duplicates, paired annotations, and recoverable operation logs. External source files are never modified. Actual third-party GUI verification remains open; models, YOLO export, and backups remain later work.
