...
- 用户必须先输入完整数据集名称；输入内容按原样、区分大小写与空格精确匹配当前名称，不自动修正；随后再进行一次明确写有“永久删除且无法恢复”的最终确认。
...
> 实现记录（2026-07-20）：上述入口、影响统计、运行时阻塞、精确名称、二次确认、UUID/重解析点边界、同卷暂存、资料库原子移除、清理待处理状态和启动恢复均已接入普通模式，并通过故障注入与其他数据集/外部文件不变回归。
