...
- Python 3.11 最新全量质量门结果为 `217 passed、1 skipped、14 warnings`；普通/预览模式均进入 Qt 事件循环。普通临时模式生成 2 个初始化文件，预览模式生成 0 个文件。根目录 `labels.txt`、未知标签默认新建、映射 UI 修复和新建标签可编辑矩形闭环属于已记录但尚未实施的下一轮整改。
