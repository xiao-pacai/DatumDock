...
`python -m datumdock` 加载真实内部资料库、图片池、标签集与标注；`python -m datumdock --ui-preview` 显示完整演示状态，并持续标记“界面预览”。预览模式不会读取或修改真实资料库。普通模式可管理标签，以 `R` 一次性画框，以 `A / D` 切图，以 `S` 确认复核，并在设置中改绑全部应用快捷键；普通滚轮竖直滚动、`Alt + 滚轮` 水平滚动、`Ctrl + 滚轮` 指针锚定缩放。AI、模型、YOLO/X-AnyLabeling 目录交换与备份入口仍明确提示后续接入。

步骤四与 A0.8 画布整改在独立 Python 3.11 `.venv` 中通过 Ruff、格式、`compileall`、`pytest-qt` 与完整回归：**201 passed、1 skipped、14 warnings**，唯一跳过仍是当前 Windows 账户缺少创建测试符号链接的权限；警告来自正式入口未使用的旧图片算法。测试覆盖 SQLite v2→v3、JSON/SQLite 故障回滚、LabelMe 混合 shape 保序、快捷键事务、两种画框手势、6400% 坐标、100%/125%/150% DPI、四边四角吸附、持续辅助线、指针锚定滚轮缩放、快速标签窗、100 图连续保存、双数据集隔离和 10,000 条分页定位。A0.8 另生成 36 张双语三分辨率/DPI 截图和 12 张 Windows 原生截图，均使用临时受管资料库。
...
The revised step-four slice and A0.5–A0.8 canvas follow-ups are implemented. The workbench now uses the same static wordmark scale as the home page with a separate back button, guides remain active in default selection mode, and Ctrl+wheel zooms around the pointer. External source files remain unchanged, and `--ui-preview` remains isolated and disposable.
...
The A0.8 baseline passes Ruff, formatting, compilation, pytest-qt, and the full Python 3.11 suite: 201 tests pass, one symlink test is skipped because the current Windows account lacks symlink privileges, and 14 legacy-image warnings remain. Fresh normal-mode event tests and bilingual resolution/DPI screenshots cover the corrected branding, guides, and wheel zoom. Model inference, YOLO export, full X-AnyLabeling directory exchange, backup, and installer delivery remain future work.
