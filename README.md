...
最新回归还覆盖整数据集删除恢复、`labels.txt` BOM/顺序/来源指纹、未知标签默认新建、外部矩形 `test1 → test2 → 下一框 test2`、保存错误分类和默认最大化。Python 3.11 最新结果为 **236 passed、1 skipped、14 warnings**；唯一跳过仍是当前 Windows 账户缺少创建测试符号链接的权限。
