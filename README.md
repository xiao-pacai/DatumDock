...
`python -m datumdock` 加载真实内部资料库、图片池、标签集与标注；`python -m datumdock --ui-preview` 显示完整演示状态，并持续标记“界面预览”。预览模式不会读取或修改真实资料库。普通模式可管理标签，以 `R` 一次性画框，以 `A / D` 切图，以 `S` 确认复核，并在设置中改绑全部应用快捷键；普通滚轮竖直滚动、`Alt + 滚轮` 水平滚动、`Ctrl + 滚轮` 指针锚定缩放。普通模式的 X-AnyLabeling 导入/导出入口已经接入真实受管服务；AI、模型、YOLO 与备份仍明确提示后续接入。

最新回归还覆盖整数据集删除恢复、`labels.txt` BOM/顺序/来源指纹、未知标签默认新建、X-AnyLabeling 四点矩形、外部矩形 `test1 → test2 → 下一框 test2`、连续三张图片分别保存、切图加载期旧画布锁定与跨样本请求拒绝、第二张图片改派后的当前行保持、标签定义表格即时写盘、普通画框前的标签修订同步、迟到加载版本保护、保存错误分类、画布/候选标签悬停零副作用、空复核值显示和默认最大化。当前测试清单为 **257 项**，完整结果为 `256 passed、1 skipped`；实机修复仍需用户按原流程复验，因此临时日志尚未关闭。其他外部阻塞记录见 [稳定化审计](docs/STABILIZATION_AUDIT.md)。当前账户缺少创建 Windows 测试符号链接的权限；覆盖率插件安装也因可信 PyPI TLS 中断受阻，均未通过关闭证书校验规避。
