...
> ✅ **已实现功能稳定化整改已接入**：X-AnyLabeling 4.x 轴对齐四点矩形会统一规范化为可编辑矩形，旋转或无法证明轴对齐的四点内容继续只读保留；双击标签改派只产生一次版本与保存请求；保存失败状态栏可打开包含请求、版本、摘要、标签修订和异常链的准确诊断。已有受管数据可从导入菜单主动执行四点矩形检查与恢复型修复。
...
最新回归还覆盖整数据集删除恢复、`labels.txt` BOM/顺序/来源指纹、未知标签默认新建、X-AnyLabeling 四点矩形、外部矩形 `test1 → test2 → 下一框 test2`、迟到加载版本保护、保存错误分类、空复核值显示和默认最大化。当前测试清单为 **248 项**；最终全量通过结果和外部阻塞记录见 [稳定化审计](docs/STABILIZATION_AUDIT.md)。当前账户缺少创建 Windows 测试符号链接的权限；覆盖率插件安装也因可信 PyPI TLS 中断受阻，均未通过关闭证书校验规避。
