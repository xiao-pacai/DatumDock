...
The revised step-four slice and A0.5–A0.7 canvas follow-ups are complete. Normal mode combines the managed library and image pool with schema-v3 review state, one-shot/two-click rectangles, 6400% inspection, 24 configurable actions, responsive quick label selection, ordered LabelMe persistence, immediate autosave, and label inspection. Persistent image-clipped guides, contextual system-pointer icons, a light canvas backplate, and rectangle input clamped from the backplate to the nearest image edge are implemented and verified. External source files remain unchanged, and `--ui-preview` remains isolated and disposable.
...
The revised step-four and canvas follow-up slices pass Ruff, formatting, compilation, pytest-qt, and the full Python 3.11 suite: 196 tests pass, one symlink test is skipped because the current Windows account lacks symlink privileges, and 14 legacy-image warnings remain. Thirty existing step-four screenshots plus 16 bilingual canvas screenshots and seven Win32 cursor-handle captures cover the target resolutions, persistent guides, high zoom, edge clamping, and every required system-pointer family. Model inference, YOLO export, full X-AnyLabeling directory exchange, backup, and installer delivery remain future work.
