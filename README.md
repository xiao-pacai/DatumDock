...
最新回归还覆盖整数据集删除恢复、`labels.txt` BOM/顺序/来源指纹、未知标签默认新建、X-AnyLabeling 四点矩形、外部矩形 `test1 → test2 → 下一框 test2`、快速新建标签后的修订同步、迟到加载版本保护、保存错误分类、空复核值显示和默认最大化。当前测试清单为 **250 项**；最终全量通过结果和外部阻塞记录见 [稳定化审计](docs/STABILIZATION_AUDIT.md)。当前账户缺少创建 Windows 测试符号链接的权限；覆盖率插件安装也因可信 PyPI TLS 中断受阻，均未通过关闭证书校验规避。
