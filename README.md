...
> ✅ **步骤三已完成**：普通模式已接入真实受管图片池，可导入六种常见静态图片、统一转为 PNG、处理完全重复决策与近似图候选，并真实分页浏览、重命名、回收站恢复或永久删除。`--ui-preview` 仍使用关闭即丢弃的内存演示数据。真实标注持久化、模型和导出逻辑尚未接入。
...
`python -m datumdock` 加载真实内部资料库与图片池；`python -m datumdock --ui-preview` 显示完整演示状态，并持续标记“界面预览”。预览模式不会读取或修改真实资料库。图片导入、缩略图浏览、重命名和删除已是真实功能；标注保存、AI、模型、YOLO/X-AnyLabeling 交换与备份入口仍明确提示后续接入，不会伪造成功。

步骤三在独立 Python 3.11 `.venv` 中通过 Ruff、格式、`compileall`、`pytest-qt` 与完整回归：**127 passed、1 skipped**，唯一跳过仍是当前 Windows 账户缺少创建测试符号链接的权限。测试覆盖 SQLite v0→v1、六种格式与 EXIF、重复/近似图、操作中断恢复、两个数据集隔离和 10,000 条分页索引。20 张原生 Windows 截图覆盖中英文、三种分辨率和步骤三核心页面。
...
推理依赖不属于步骤三的启动前提；后续接入 ONNX/PT 时再执行 `python -m pip install -e ".[dev,inference]"`。
...
| [受管图片池说明](docs/IMAGE_POOL.md) | 导入转码、哈希与重复图、SQLite v1、分页缩略图、重命名、回收站和崩溃恢复边界。 |
...
| [UI 与步骤三页面清单](docs/UI_INVENTORY.md) | 16 个路由、28 个弹窗、真实资料库/图片池入口、预览边界与验证结果。 |
| [UI 与步骤三交付报告](docs/UI_REVIEW.md) | 截图矩阵、验证命令、步骤三边界与自评。 |
...
Step three is complete. In addition to the persistent UUID-backed managed library, normal mode now ingests six common static image formats into normalized managed PNGs, provides explicit exact-duplicate decisions and reviewable near-image candidates, and supports paged thumbnails, a real canvas, batch rename, Dataset Trash restore, and permanent deletion. External source files remain unchanged. `python -m datumdock --ui-preview` remains isolated and disposable.
...
The step-three slice passes Ruff, formatting, compilation, pytest-qt, and the full Python 3.11 suite: 127 tests pass and one symlink test is skipped because the current Windows account lacks symlink privileges. Twenty bilingual native screenshots cover 1366×768, 1440×900, 1920×1080, the real image workspace, similarity review, trash, settings, and import UI. Persistent annotations, model inference, dataset export, backup, and installer delivery remain future work.
