...
步骤二修复复验已在独立 Python 3.11 `.venv` 中完成：Ruff、格式检查、`compileall`、`pytest-qt` 和完整测试均通过，结果为 **88 passed、1 skipped**。唯一跳过项是当前 Windows 账户没有创建符号链接的权限；实现仍明确拒绝跟随符号链接，未知目录保留测试已通过。普通/预览 GUI 均进入真实 Qt 事件循环，12 张中英文原生截图覆盖 1366×768、1440×900、1920×1080，真实 `%LOCALAPPDATA%\DatumDock` 复验前后哈希一致。
...
The repaired managed-library slice has passed Ruff, formatting, compilation, pytest-qt, and the full suite in an isolated Python 3.11 environment: 88 tests passed and one symlink test was skipped because the current Windows account lacks symlink privileges. Normal and preview GUI event-loop smoke tests pass, twelve bilingual native screenshots cover 1366×768, 1440×900, and 1920×1080, and the real `%LOCALAPPDATA%\DatumDock` tree hash is unchanged. Image import, persistent annotations, model inference, dataset export, backup, and installer delivery remain future work.
