...
步骤五新增回归覆盖安全递归扫描、图片/JSON 配对、来源指纹、标签映射、重复决策、LabelMe 混合 shape 与未知 shape 无损往返、无标注图片不生成 JSON、递归私有字段清理、SQLite/文件故障回滚、任务数据集隔离和 100 图导入/重开/导出。Python 3.11 最新结果为 **217 passed、1 skipped、14 warnings**；唯一跳过仍是当前 Windows 账户缺少创建测试符号链接的权限。
...
| 领域 | 规划能力 |
| --- | --- |
| 数据集主页 | **步骤二已实现**：像游戏存档一样创建、保存、搜索、排序、打开、切换、重命名、归档和恢复多个受管数据集；下一轮将接入带影响预览、名称确认和恢复日志的整数据集永久删除。 |
