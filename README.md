...
> ✅ **步骤四及 A0.5～A0.7 画布整改已完成**：普通模式已接入 schema v3 双状态复核、一次性矩形、两点/拖拽画框、6400% 检查、全量可配置快捷键、响应式快速标签窗和 LabelMe 立即自动保存。画布辅助线在图片内持续跟随，移动/缩放使用对应系统指针，中央区域使用浅色底板，矩形输入落在底板时安全吸附到图片边缘。`--ui-preview` 仍使用关闭即丢弃的内存演示数据。模型推理、YOLO 导出、完整 X-AnyLabeling 目录互操作和备份尚未接入。
...
> 🛠️ **A0.8 用户实机整改已锁定，代码待重新实施与复验**：标注工作台左上角完整 Logo 需要显著放大；十字辅助线必须在默认选择模式、未选中方框和一次性画框结束后仍持续跟随鼠标；`Ctrl + 滚轮` 改为以鼠标所在图片位置为锚点放大/缩小。A0.5 的旧自动化与截图证据保留为历史，但不能替代本轮普通模式实机验收。
...
`python -m datumdock` 加载真实内部资料库、图片池、标签集与标注；`python -m datumdock --ui-preview` 显示完整演示状态，并持续标记“界面预览”。预览模式不会读取或修改真实资料库。普通模式可管理标签，以 `R` 一次性画框，以 `A / D` 切图，以 `S` 确认复核，并在设置中改绑全部应用快捷键；A0.8 将补齐普通滚轮竖直滚动、`Alt + 滚轮` 水平滚动和 `Ctrl + 滚轮` 指针锚定缩放。AI、模型、YOLO/X-AnyLabeling 目录交换与备份入口仍明确提示后续接入。

步骤四与画布整改在独立 Python 3.11 `.venv` 中通过 Ruff、格式、`compileall`、`pytest-qt` 与完整回归：**196 passed、1 skipped、14 warnings**，唯一跳过仍是当前 Windows 账户缺少创建测试符号链接的权限；警告来自正式入口未使用的旧图片算法。测试覆盖 SQLite v2→v3、JSON/SQLite 故障回滚、LabelMe 混合 shape 保序、快捷键事务、两种画框手势、6400% 坐标、100%/125%/150% DPI、四边四角吸附、快速标签窗、100 图连续保存、双数据集隔离和 10,000 条分页定位。既有 30 张步骤四截图之外，A0.5～A0.7 新增 16 张双语画布截图和 14 张中英文 Win32 原生系统光标截图。
...
The revised step-four slice and the historical A0.5–A0.7 canvas follow-ups are implemented. A0.8 is now specified but not yet reimplemented or reverified after hands-on review: the workbench wordmark must be visibly larger, guides must remain active in the default unarmed selection state, and Ctrl+wheel must zoom around the pointer. External source files remain unchanged, and `--ui-preview` remains isolated and disposable.
...
The pre-A0.8 baseline passes Ruff, formatting, compilation, pytest-qt, and the full Python 3.11 suite: 196 tests pass, one symlink test is skipped because the current Windows account lacks symlink privileges, and 14 legacy-image warnings remain. Those results are historical evidence only; A0.8 requires new normal-mode interaction tests and screenshots before completion can be claimed. Model inference, YOLO export, full X-AnyLabeling directory exchange, backup, and installer delivery remain future work.
